...
export function getChatConfig(chatId: number): ChatApiConfig {
  const row = getDb()
    .prepare<[number], ConfigRow>("SELECT api_key AS apiKey, base_url AS baseUrl FROM chat_configs WHERE chat_id = ?")
    .get(chatId);
  if (!row) return {};
  return {
...
export async function resetChatApiKey(chatId: number): Promise<void> {
  ...
  getDb()
    .prepare("DELETE FROM chat_configs WHERE chat_id = ? AND api_key IS NULL AND base_url IS NULL")
    .run(chatId);
}
